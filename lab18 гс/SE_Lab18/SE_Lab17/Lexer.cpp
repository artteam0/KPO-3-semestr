...
namespace  Lexer
{
	...
	void checkElInIdTable(std::vector<std::pair<std::string, int>> words, int i, LT::LexTable& lextable, IT::IdTable& idtable, int indIdTab, int counter)
	{
		IT::Entry ent;
		...
		if (LT::GetEntry(lextable, i - 1).lexema == 'f')
		{
			if (IT::IsId(idtable, words[i].first) == TI_NULLIDX)
			{
				...
				{
					ent = { i + 1, words[i].first, IT::INT, IT::F, TI_INT_DEFAULT, counter };
				}
				else if (words[i - 2].first == "string") {

					ent = { i + 1, words[i].first, IT::STR, IT::F, TI_STR_DEFAULT , counter};
				}
				else {
					ERROR_THROW(121);
				}
		...
		else if (LT::GetEntry(lextable, i - 1).lexema == 't' && LT::GetEntry(lextable, i - 2).lexema == 'd') //если предыдущие идент. d и t, то инд. считается переменной
		{
			...
			if ((IT::IsId(idtable, words[i].first) == TI_NULLIDX || idtable.table[buf].idtype == IT::P) || (IT::IsId(idtable, words[i].first) != TI_NULLIDX && isFunc))
			{
				...
				{
					ent = { i + 1, words[i].first, IT::INT, IT::V, TI_INT_DEFAULT, counter };
				}
				else if (words[i - 1].first == "string")
				{
					ent = { i + 1, words[i].first, IT::STR, IT::V, TI_STR_DEFAULT, counter };
				}
				else {
					ERROR_THROW(121);
				}
		...
		else if (LT::GetEntry(lextable, i - 2).lexema == ',' || LT::GetEntry(lextable, i - 4).lexema == 'f')
		{
			...
			if (words[i - 1].first == "integer")
			{
				ent = { i+1, words[i].first, IT::INT, IT::P, TI_INT_DEFAULT, counter };
			}
			else if (words[i - 1].first == "string")
			{
				ent = { i+1,words[i].first,IT::STR,IT::P,TI_STR_DEFAULT, counter };
			}
			else {
				ERROR_THROW(121);
			}
	...
	{
		if (lextable.table[ind - 1].lexema == '=' && lextable.table[ind - 2].lexema == 'i') //присваивание значения
		{
			int buf = IT::IsId(idtable, words[ind - 2].first);
			idtable.table[buf].value.vint = std::stoi(words[ind].first); //преобразование строки в int
		}
	}
	...
	void valueOfSTR(LT::LexTable& lextable, IT::IdTable& idtable, int ind, std::vector<std::pair<std::string, int>> words)
	{
		if (lextable.table[ind - 1].lexema == '=' && lextable.table[ind - 2].lexema == 'i') //присваивание значения
		{
			int buf = IT::IsId(idtable, words[ind - 2].first); //получение индекса
			std::string strValue = words[ind].first;
			idtable.table[buf].value.vstr->len = (char)(strValue.length()); //запись длины строки(len)
			strncpy(idtable.table[buf].value.vstr->str, strValue.c_str(), TI_STR_MAXSIZE - 1); //копирование строки(TI_STR_MAXSIZE - 1 для избегания переполнения)
		}
	}
	...
	void Run(LT::LexTable& lextable, IT::IdTable& idtable, In::ININ in) {
		...
		for (int i = 0;i < size;i++)
		{
			...
			else if (check(words[i].first, typeIdentificator)) 
			{
				...
				checkElInIdTable(words, i, lextable, idtable, indIdTab, counter);
		...
		currLine = 0;
		for (int i = 0; i < size; i++) {
			if (words[i].second != currLine) { //если текущая не совпадает с curr
				currLine++;
				cout << "\n";
				cout << setw(2) << setfill('0') << currLine << " "; //номер строки с лидирующими нулями
		...
		cout << left << setw(10) << setfill(' ') << "ID" << setw(10) << "Тип" << setw(22) << "Тип данных"
		<< setw(5) << "id" << setw(20) << "Области видимости" << setw(10) << "Значение" << endl;
		cout << "----------------------------------------------------------------------------------" << endl;

		for (int i = 0; i < idtable.size; i++) {
			IT::Entry ent = IT::GetEntry(idtable, i);
			cout << setw(11) << setfill(' ') << ent.id;

			// Тип переменной (V, F, P)
			cout << setw(12) << (ent.idtype == 1 ? "V" : (ent.idtype == 2 ? "F" : "P"));

			// Тип данных (INT, STR)
			cout << setw(19) << (ent.iddatatype == IT::INT ? "INT" : "STR");

			cout << setw(12) << ent.idxfirstLE << " ";
			//области видимости
			cout << setw(15) << ent.counter << " ";
